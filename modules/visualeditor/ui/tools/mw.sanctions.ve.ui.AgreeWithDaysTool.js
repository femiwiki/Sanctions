( function ( mw, OO, ve ) {
	...
	mw.sanctions.ve.ui.AgreeWithDaysTool = function SanctionsVeUiAgreeWithDaysInspectorTool() {
		var self = this;
		$( function () {
			var title = self.$element.closest( '.flow-topic' ).find( '.flow-topic-title' ).text();
			if ( title.match( new RegExp( mw.config.get( 'wgSanctionsInsultingNameTopicTitle' ) ) ) ) {
				self.destroy();
			}
		} );
		// Parent constructor
	 ...
	 * @return {boolean}
	 */
	mw.sanctions.ve.ui.AgreeWithDaysTool.static.isCompatibleWith = function ( model ) {
		return model instanceof ve.dm.MWTransclusionNode &&
			model.isSingleTemplate( mw.sanctions.ve.ui.AgreeWithDaysTool.static.template );
	};
	...
	ve.ui.commandRegistry.register(
		new ve.ui.Command(
			...
			'window',
			'open',
			{ args: [ 'sanctions-agree' ], supportedSelections: [ 'linear' ] }
		)
	);
