...
class SanctionsUtils {
	...
	public static function hasVoteRight ( User $user, &$reason = false ) {
		...
		//현재 편집 권한이 없을 경우 불가
		if( !$user->isAllowed('edit') ) if($reason!==false) $reason[] = '편집 권한이 없음'; else return false;
		...
		if ($twentyDaysAgo < $reg) if($reason!==false) $reason[] = '가입한 지 '.$verificationPeriod.'일이 경과하지 않음('.MWTimestamp::getLocalInstance($reg)->getTimestamp(TS_ISO_8601).'에 가입)'; else return false;
		...
		if ( $count < $verificationEdits ) if($reason!==false) $reason[] = '최근 '.$verificationPeriod .'일 간 편집 수가 '.$count.'번으로 '.$verificationEdits.'번보다 작음'; else return false;
